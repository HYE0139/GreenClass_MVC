const ws = new WebSocket("ws://192.168.0.35:8090/");
...
(function() {
    ...
    if(gData && gData.dataset.loginiuser) {
        ...
        ws.onmessage = function(e) {
            var json = JSON.parse(e.data);// 문자열로 넘어온 걸 JSON 객체로 만듦
            console.log(json);            
            ...
            switch(json.type) {
                case 'dm':                        
                    ...
                    if(inputIdm) {  //현재 페이지가 dm페이지라면
                        ...
                        } else { // dm페이지에서 대화 중일때 다른 유저가 메세지를 보냈다면
                            
                        }
                      
                    } else { //현재 페이지가 dm페이지가 아니라면, header에 있는 dm아이콘 작업
                       if(loginUser !== json.toiuser) { return; }
