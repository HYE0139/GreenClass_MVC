(function() {
    ...
    // searchParams : URL 내의 GET 디코딩 된 쿼리 매개변수에 접근
    // oppoiuser = feed 를 만들 때 저장된 iuser 
    let oppoiuser = urlObj.searchParams.get('oppoiuser') ? parseInt(urlObj.searchParams.get('oppoiuser')) : 0;
     // 쿼리스트링에 oppoiuser(iuser)가 있으면 iuser를 숫자값으로 저장, 아니면 0을 저장
    ...
    // 대화 목록 중에 해당 유저가 없다면(isNonExistent = true;)->새로운 DM 방 만들기
    function makeDmUserList(userList) {
        ...
        userList.forEach(function(item) {
            ...
            // 쿼리스트링의 유저 === 디엠방 리스트의 유저
            if(oppoiuser === item.opponent.iuser) {
                isNonExistent = false; // false를 저장
                getDmMsgList(item.idm); // DB에 저장된 디엠 불러오기
                console.log(ws);                
            }
        });
        
        if(oppoiuser !== 0 && isNonExistent) { // iuser값이 0이 아니고, true 이면
            fetch('/dm/reg', {
                method: 'POST',
                body: JSON.stringify({
                    'toiuser': oppoiuser
            ...
            .then(res => {                
                setIdm(res.idm);
                const div = makeDmUserItem(res); //DB에 저장된 메세지
                dmUserListContainerElem.append(div); //메세지를 추가                    
            });
        }
    }
    function makeDmUserItem(item) { //새로운 디엠방 만들기
